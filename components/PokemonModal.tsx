...
export default function PokemonModal({ pokemonName, onClose }: { pokemonName: string | null, onClose: () => void}) {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex justify-center items-center p-4">
            <div className="bg-slate-800 border border-slate-700 w-full max-w-lg rounded-2xl overflow-hidden relative">
                <button onClick={onClose} className="absolute top-4 right-4 text-white text-2xl cursor-pointer border border-slate-700 rounded rounded-md pl-2 pr-2">X</button>

                {loading ? (
                    <div className="p-20 text-center text-yellow-400 animate-pulse">Loading...</div>
                ) : (
